...
using hand_pipeline::Detection;
using hand_pipeline::GtAnnotation;
using hand_pipeline::ImageRecord;
using hand_pipeline::Keypoint;
using hand_pipeline::kNumKpts;
...
int main(int argc, char** argv) {
    ...
    hand_pipeline::DetConfig dcfg;
    dcfg.score_thr = args.score_thr;
    dcfg.nms_iou_thr = args.nms_thr;
    hand_pipeline::PoseConfig pcfg;

    hand_pipeline::RtmDet det(args.det_path, dcfg);
    hand_pipeline::RtmPose pose(args.pose_path, pcfg);

    const std::string ann_path = args.eval_dir + "/" + kEvalAnnName;
    const std::string img_dir  = args.eval_dir + "/" + kEvalImagesName;

    auto records = hand_pipeline::LoadCocoEval(ann_path, img_dir);
    if (records.empty()) {
    ...
    for (const auto& li : imgs) {
        ...
        cv::Mat vis = hand_pipeline::viz::DrawHands(
            li.img, preds, kpts_per_pred, viz_label);
        cv::imwrite(viz_dir + "/" + li.fname, vis);

        // 匹配 + PCK
        const auto pairs = hand_pipeline::MatchPredsToGt(preds, li.rec->gts);
